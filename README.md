...
- **문제**: 초기 데이터셋을 실사 영상에서 추출한 프레임 위주로 구성했습니다. 촬영 과정에서 화면이 좌우 반전되거나 기울어지고, 원근 왜곡과 반사가 포함되면서 일부 UI 요소의 탐지 성능이 불안정했습니다.
...
- **결과**: 디지털 원본과 실사 데이터를 함께 사용하는 학습 구성을 확립했고, 최종 모델은 `mAP@0.5 98.7%`를 기록했습니다. 데이터 추가 전후의 성능 차이는 별도로 측정하지 않아 개선률은 제시하지 않았습니다.
