...
#### 실사 프레임 중심 데이터셋의 탐지 성능 저하

- **문제**: 초기 데이터셋을 실사 영상에서 추출한 프레임 위주로 구성했습니다. 일부 촬영 프레임이 좌우 반전된 상태로 저장됐고, 기울기·원근 왜곡·반사까지 포함되면서 일부 UI 요소의 탐지 성능이 불안정했습니다.
- **원인**: 실제 환경 변화는 충분히 포함됐지만, UI 컴포넌트의 기준 형태를 명확하게 보여주는 원본 이미지가 부족해 클래스별 시각적 특징을 안정적으로 학습하기 어려웠습니다.
- **해결**: 키오스크 UI의 디지털 원본 캡처를 추가하고, 실사 촬영 이미지와 함께 학습하도록 데이터셋을 재구성했습니다. 디지털 원본은 기준 형태를 제공하고, 실사 데이터는 반사·기울기·원근 왜곡 등 환경 변화를 보완하도록 역할을 나눴습니다.
- **결과**: 디지털 원본과 실사 데이터를 함께 사용하는 학습 구성을 확립했고, 최종 모델은 `mAP@0.5 98.7%`를 기록했습니다.

### 데이터셋과 탐지 모델

- 디지털 키오스크 캡처와 실사 촬영 데이터를 정제해 **387장**으로 구성
- YOLOv8n을 100 epoch, 640px 입력 크기로 학습
- UI 요소 8개 클래스를 학습했으며, 앱에서는 텍스트 정보는 OCR로 처리하고 YOLOv8n은 버튼·옵션·팝업 등 UI 영역 탐지에 활용
