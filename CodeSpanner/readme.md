...
OpenCV의 Lucas-Kanade 알고리즘 기반 주적 기술을 활용하여 연속적인 프레임 간의 카메라 이동량을 계산합니다. 사용자가 키오스크 앞에서 손을 미세하게 떨더라도, 기존에 탐지된 바운딩 박스들이 흔들림에 따라 밀려나지 않고 제자리를 유지하도록 좌표를 실시간 보정합니다.
- MenuMatcher.kt
...
B. 실시간 인터랙션 및 피드백 파이프라인 (INTERACTION 상태 유지 시)
